...
\begin{titlepage}
    ...
    \textbf{Notebooks :} \texttt{07\_demo\_*.ipynb}
    }
...
\begin{tikzpicture}[scale=0.8]
    ...
    \foreach \i in {0,...,4} {
        \foreach \j in {0,...,4} {
            \draw[fill=blue!10] (\j, -\i) rectangle ++ (1, 1);
        }
    }
    \node[above, font=\small\bfseries] at (2.5, 0.3) {Image d'entrée $\mat{I}$ (5×5)};

    % Highlight first receptive field (3x3)
    \foreach \i in {0,...,2} {
        \foreach \j in {0,...,2} {
            \draw[fill=green!30, opacity=0.7] (\j, -\i) rectangle ++ (1, 1);
        }
    }
    ...
    \begin{scope}[xshift=7cm]
        \foreach \i in {0,...,2} {
            \foreach \j in {0,...,2} {
                \draw[fill=orange!40] (\j, -\i) rectangle ++ (1, 1);
            }
        }
        \node[above, font=\small\bfseries] at (1.5, 0.3) {Filtre $\mat{K}$ (3×3)};

        % Kernel values example
        \node[font=\tiny] at (0.5, -0.5) {$1$};
        \node[font=\tiny] at (1.5, -0.5) {$0$};
        \node[font=\tiny] at (2.5, -0.5) {$-1$};
        \node[font=\tiny] at (0.5, -1.5) {$1$};
        \node[font=\tiny] at (1.5, -1.5) {$0$};
        \node[font=\tiny] at (2.5, -1.5) {$-1$};
    ...
    \draw[->, very thick, red!70!black] (5.5, -1) -- (6.5, -1)
    ...
    \begin{scope}[xshift=11cm]
        \foreach \i in {0,...,2} {
            \foreach \j in {0,...,2} {
                \draw[fill=purple!20] (\j, -\i) rectangle ++ (1, 1);
            }
        }
        \node[above, font=\small\bfseries] at (1.5, 0.3) {Feature Map $\mat{O}$ (3×3)};

        % First value highlighted
        \draw[fill=red!40, opacity=0.7] (0, 0) rectangle ++ (1, 1);
        \node[font=\small, red!70!black] at (0.5, -0.5) {$\mat{O}_{00}$};
    \end{scope}

    % Sliding window illustration (bottom)
    \begin{scope}[yshift=-7cm]
        % Step 1: Position (0,0)
        \node[font=\small\bfseries] at (2.5, 0.5) {Étape 1: Position (0,0)};
        \foreach \i in {0,...,4} {
            \foreach \j in {0,...,4} {
                \draw[fill=blue!10] (\j, -\i) rectangle ++ (1, 1);
            }
        ...
        \node[red, font=\tiny] at (1.5, -3.3) {Stride = 1};

        % Step 2: Position (0,1)
        \begin{scope}[xshift=6cm]
            \node[font=\small\bfseries] at (2.5, 0.5) {Étape 2: Position (0,1)};
            \foreach \i in {0,...,4} {
                \foreach \j in {0,...,4} {
                    \draw[fill=blue!10] (\j, -\i) rectangle ++ (1, 1);
                }
            ...
            \draw[red, very thick] (1, 0) rectangle (4, -3);
            \draw[->, thick, blue] (-0.5, -1.5) -- (0.8, -1.5);
        \end{scope}

        % Step 3: Position (1,0)
        \begin{scope}[yshift=-6cm]
            \node[font=\small\bfseries] at (2.5, 0.5) {Étape 3: Position (1,0)};
            \foreach \i in {0,...,4} {
                \foreach \j in {0,...,4} {
                    \draw[fill=blue!10] (\j, -\i) rectangle ++ (1, 1);
                }
            ...
            \draw[red, very thick] (0, -1) rectangle (3, -4);
            \draw[->, thick, blue] (1.5, 0.5) -- (1.5, -0.8);
        \end{scope}

        % Output grid showing positions
        \begin{scope}[xshift=6cm, yshift=-6cm]
            \node[font=\small\bfseries] at (2.5, 0.5) {Feature Map (3×3)};
            \foreach \i in {0,...,2} {
                \foreach \j in {0,...,2} {
                    \draw[fill=purple!20] (\j*1.5, -\i*1.5) rectangle ++ (1.2, 1.2);
                }
            }
            % Label positions
            \node[font=\tiny, red] at (0.6, -0.6) {(0,0)};
            \node[font=\tiny, blue] at (2.1, -0.6) {(0,1)};
            \node[font=\tiny] at (3.6, -0.6) {(0,2)};
            \node[font=\tiny, blue] at (0.6, -2.1) {(1,0)};
            \node[font=\tiny] at (2.1, -2.1) {(1,1)};
            \node[font=\tiny] at (3.6, -2.1) {(1,2)};
            \node[font=\tiny] at (0.6, -3.6) {(2,0)};
            \node[font=\tiny] at (2.1, -3.6) {(2,1)};
            \node[font=\tiny] at (3.6, -3.6) {(2,2)};
        \end{scope}
    \end{scope}

\end{tikzpicture}
\caption{Opération de convolution 2D avec sliding window (fenêtre glissante). \textbf{Haut:} Vue d'ensemble: le filtre 3×3 glisse sur l'image 5×5 pour produire une feature map 3×3. À chaque position, on calcule le produit scalaire entre le filtre et la zone correspondante de l'image (en vert). \textbf{Bas:} Détail du sliding window avec stride=1: le filtre se déplace d'un pixel à la fois (horizontalement puis verticalement), calculant une valeur de sortie pour chaque position. Avec une image 5×5, un filtre 3×3, et sans padding, on obtient une sortie 3×3.}
...
\begin{tikzpicture}[
    scale=0.7,
    layer/.style={rectangle, draw, minimum height=3cm, fill=blue!20, font=\tiny},
    conv/.style={rectangle, draw, minimum height=3cm, fill=green!30, font=\tiny},
    pool/.style={rectangle, draw, minimum height=2.5cm, fill=orange!30, font=\tiny},
    fc/.style={rectangle, draw, minimum height=2cm, fill=purple!30, font=\tiny}
]

    % Input
    \node[layer, minimum width=0.8cm, minimum height=3.2cm, fill=gray!20] (input) at (0,0) {
        \rotatebox{90}{Input}
    };
    \node[below, font=\tiny, align=center] at (input.south) {32×32×1};

    % C1: Conv 5x5, 6 filters
    \node[conv, minimum width=1.2cm, right=0.5cm of input] (c1) {
        \rotatebox{90}{Conv 6@5×5}
    };
    \node[below, font=\tiny, align=center] at (c1.south) {28×28×6};
    \draw[->, thick] (input.east) -- (c1.west);

    % S2: AvgPool 2x2
    \node[pool, minimum width=1cm, right=0.5cm of c1] (s2) {
        \rotatebox{90}{AvgPool 2×2}
    };
    \node[below, font=\tiny, align=center] at (s2.south) {14×14×6};
    \draw[->, thick] (c1.east) -- (s2.west);

    % C3: Conv 5x5, 16 filters
    \node[conv, minimum width=1.4cm, right=0.5cm of s2] (c3) {
        \rotatebox{90}{Conv 16@5×5}
    };
    \node[below, font=\tiny, align=center] at (c3.south) {10×10×16};
    \draw[->, thick] (s2.east) -- (c3.west);

    % S4: AvgPool 2x2
    \node[pool, minimum width=0.8cm, right=0.5cm of c3] (s4) {
        \rotatebox{90}{AvgPool 2×2}
    };
    \node[below, font=\tiny, align=center] at (s4.south) {5×5×16};
    \draw[->, thick] (c3.east) -- (s4.west);

    % C5: Conv 5x5, 120 filters
    \node[conv, minimum width=0.6cm, right=0.5cm of s4] (c5) {
        \rotatebox{90}{Conv 120}
    };
    \node[below, font=\tiny, align=center] at (c5.south) {1×1×120};
    \draw[->, thick] (s4.east) -- (c5.west);

    % F6: Fully Connected 84
    \node[fc, minimum width=1cm, right=0.5cm of c5] (f6) {
        \rotatebox{90}{FC 84}
    };
    \node[below, font=\tiny, align=center] at (f6.south) {84};
    \draw[->, thick] (c5.east) -- (f6.west);

    % Output: FC 10
    \node[fc, minimum width=1cm, minimum height=1.5cm, right=0.5cm of f6, fill=red!30] (output) {
        \rotatebox{90}{Output 10}
    };
    \node[below, font=\tiny, align=center] at (output.south) {10 classes};
    \draw[->, thick] (f6.east) -- (output.west);

    % Legend
    \node[above, font=\small\bfseries] at (7, 4) {Légende:};
    \node[conv, minimum width=1.5cm, minimum height=0.7cm] at (7, 3.2) {Convolution};
    \node[pool, minimum width=1.5cm, minimum height=0.7cm] at (7, 2.4) {Pooling};
    \node[fc, minimum width=1.5cm, minimum height=0.7cm] at (7, 1.6) {Fully Connected};

    % Annotations
    \draw[dashed, gray] (input.north) ++ (0, 0.3) -- ++ (13, 0);
    \node[above, font=\footnotesize, align=center] at (6.5, 3.8) {Feature Extraction (Convolutional Layers)};

    \draw[dashed, gray] (f6.north) ++ (-0.5, 0.3) -- ++ (3, 0);
    \node[above, font=\footnotesize, align=center] at (11, 2.5) {Classification};

\end{tikzpicture}
\caption{Architecture LeNet-5 (LeCun et al., 1998). \textbf{Structure:} 2 blocs [Conv + AvgPool] pour l'extraction de features, suivis de 3 couches fully-connected pour la classification. LeNet-5 a été le premier CNN à réussir sur MNIST (reconnaissance de chiffres manuscrits) avec une précision >99\%. \textbf{Innovations:} Partage de poids (convolution), pooling spatial, architecture hiérarchique. \textbf{Note:} À l'époque, la fonction d'activation était tanh; aujourd'hui on utiliserait ReLU.}
...
\textit{Solutions disponibles dans} \texttt{07\_exercices.ipynb} \textit{(solutions intégrées dans le notebook)}
